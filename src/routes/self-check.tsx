...
const SCENARIO = {
  ...
  best: 2,
  expected: "Strong choice surfaces the blocker early and shares ownership — and mentors also expect you to document the new plan.",
};
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground"><Lock className="h-3.5 w-3.5" /> Private</span>}
      />

      <div className="mb-5 flex items-center gap-2 text-xs font-medium text-muted-foreground">
        {[1, 2, 3].map((n) => (
          <div key={n} className="flex items-center gap-2">
            <span className={`grid h-6 w-6 place-items-center rounded-full text-[11px] ${step >= n ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{n}</span>
            {n < 3 && <span className={`h-px w-8 ${step > n ? "bg-primary" : "bg-border"}`} />}
          </div>
        ))}
        <span className="ml-2">{step === 1 ? "Scenario" : step === 2 ? "Rate yourself" : "Your blind spots"}</span>
      </div>

      {step === 1 && (
        <Card className="max-w-2xl cgt-rise">
          <CardHeader title="Scenario" description="Answer honestly — there is an expert-preferred response." />
          <div className="p-5">
            <p className="font-medium">{SCENARIO.prompt}</p>
            <div className="mt-4 space-y-2">
              {SCENARIO.options.map((o, i) => (
                <label key={i} className={`flex cursor-pointer items-center gap-3 rounded-md border px-3 py-2.5 text-sm transition-colors ${answer === i ? "border-primary bg-accent/50" : "border-border hover:bg-accent/40"}`}>
                  <input type="radio" name="ans" checked={answer === i} onChange={() => setAnswer(i)} className="accent-[color:var(--primary)]" />
                  {o}
                </label>
              ))}
            </div>
            <div className="mt-5 flex justify-end">
              <button onClick={() => setStep(2)} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
        ...
        <Card className="max-w-2xl cgt-rise">
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex justify-between pt-2">
              <button onClick={() => setStep(1)} className="inline-flex items-center gap-1.5 rounded-md px-3 py-2 text-sm text-muted-foreground hover:bg-accent">
                <ArrowLeft className="h-4 w-4" /> Back
              </button>
              <button onClick={() => setStep(3)} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
      ...
      {step === 3 && (
        <div className="space-y-4 max-w-2xl">
          <Card className="cgt-rise">
            ...
            <div className="p-5 text-sm">
              <Pill tone={answer === SCENARIO.best ? "strong" : "developing"}>
                {answer === SCENARIO.best ? <><Check className="mr-1 inline h-3 w-3" />Well calibrated</> : "Worth reflecting on"}
              </Pill>
          ...
          <Card className="cgt-rise" >
            <CardHeader title="Self vs mentor" description="The gap between your self-rating and validated mentor scores — private to you." />
