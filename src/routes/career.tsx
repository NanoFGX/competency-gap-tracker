...
function CareerPage() {
  ...
  const STUDENT_COLORS = chart.series;
  ...
  const activeStudent =
    role === "recruiter"
      ? (students.find((s) => s.id === compareStudentId) ?? ctxStudent)
      : ctxStudent;
  ...
  const priority = COMPETENCIES.map((c) => ({
    competency: c,
    gap: Math.max(0, target[c] - scores[c]),
  }))
    .filter((d) => d.gap > 0)
  ...
  const avgStudentScore = (
    COMPETENCIES.reduce((s, c) => s + scores[c], 0) / COMPETENCIES.length
  ).toFixed(1);
  const avgTargetScore = (
    COMPETENCIES.reduce((s, c) => s + target[c], 0) / COMPETENCIES.length
  ).toFixed(1);
  const readinessPct = Math.round(
    (COMPETENCIES.reduce((s, c) => s + scores[c], 0) / (COMPETENCIES.length * 10)) * 100,
  );
  ...
  const earliestReadiness = earliestEval
    ? Math.round(
        (COMPETENCIES.reduce((s, c) => s + earliestEval.scores[c], 0) /
  ...
  const cohortRows = students.map((s, i) => {
    ...
    const rdy = Math.round(
      (COMPETENCIES.reduce((sum, c) => sum + sc[c], 0) / (COMPETENCIES.length * 10)) * 100,
    );
    return {
      id: s.id,
      name: s.name,
      coverage: Math.round((met / COMPETENCIES.length) * 100),
      readiness: rdy,
      scores: sc,
      color: STUDENT_COLORS[i],
    };
  });
  const cohortAvgCoverage = Math.round(
    cohortRows.reduce((s, d) => s + d.coverage, 0) / cohortRows.length,
  );
  ...
  const cohortRank =
    [...cohortRows]
      ...
      .findIndex((r) => r.id === activeStudent.id) + 1;
  ...
  return (
    <>
      <PageHeader
        eyebrow={role === "recruiter" ? "Recruiter" : "Student"}
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <StatCard
              ...
              sub={`benchmarked vs ${selectedRole}`}
            />
            <StatCard
              icon={<CheckCircle2 className="h-4 w-4 text-[color:var(--success)]" />}
              label="Cohort avg coverage"
              value={`${cohortAvgCoverage}%`}
              sub="competency requirements met"
            />
            <StatCard
              icon={<TrendingUp className="h-4 w-4 text-[color:var(--success)]" />}
              label="Top candidate"
              value={cohortRows.sort((a, b) => b.coverage - a.coverage)[0].name.split(" ")[0]}
              sub={`${cohortRows.sort((a, b) => b.coverage - a.coverage)[0].coverage}% coverage`}
            />
            <StatCard
              icon={<Target className="h-4 w-4 text-muted-foreground" />}
              label="Avg readiness"
              value={`${Math.round(cohortRows.reduce((s, d) => s + d.readiness, 0) / cohortRows.length)}%`}
          ...
          <Card className="mb-4">
            <CardHeader
              ...
              description={`All candidates vs ${selectedRole} requirements`}
            />
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                        ...
                        {COMPETENCIES.map((c) => {
                          ...
                          return (
                            <td key={c} className="px-3 py-3 text-center hidden md:table-cell">
                              <span
                                className={`font-mono text-xs ${met ? "text-[color:var(--success)]" : "text-destructive"}`}
                              >
                                {v}/{t}
                              </span>
                            </td>
                  ...
                  <tr className="bg-muted/20 text-muted-foreground">
                    <td className="px-5 py-3 text-xs font-medium uppercase tracking-wider">
                      {selectedRole} target
                    </td>
                    {COMPETENCIES.map((c) => (
                      <td key={c} className="px-3 py-3 text-center hidden md:table-cell">
          ...
          <Card>
            <CardHeader
              title="Gap Heatmap"
              description="How far each candidate is from the role requirement per competency"
            />
                      ...
                      return (
                        <div key={r.id} className="flex-1 flex flex-col items-center gap-1">
                          <div className="relative w-full h-3 rounded bg-muted border border-border">
                            ...
                            />
                            <div
                              className="absolute -top-0.5 -bottom-0.5 w-0.5 bg-foreground/40"
                              style={{ left: `${(t / 10) * 100}%` }}
                            />
                          </div>
                          <span
                            className={`text-[10px] font-mono ${met ? "text-[color:var(--success)]" : "text-muted-foreground"}`}
                          >
                            {r.name.split(" ")[0]}
                          </span>
                        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
            <StatCard
              ...
              tip={
                role === "recruiter"
                  ? "Based on requirements met (≥67%), readiness (≥70%), and zero critical gaps."
                  : "Reflects whether your current profile meets the coverage and readiness thresholds for the selected role."
              }
            ...
            <StatCard
              ...
              sub={
                totalGapPoints === 0
                  ? "Profile fully meets role"
                  : `across ${missing.length} competencies`
              }
            />
            <StatCard
              ...
              sub={
                coveragePct >= cohortAvgCoverage ? "Above cohort average" : "Below cohort average"
              }
            />
          </div>
              ...
              <div className="p-5 h-[320px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={singleData} margin={{ top: 5, right: 8, left: -16, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={chart.tooltip} />
                    <Legend wrapperStyle={{ fontSize: 12 }} />
                    <Bar dataKey={activeStudent.name} fill={chart.primary} radius={[3, 3, 0, 0]} />
                    <Bar
                      dataKey={`${selectedRole} target`}
                      fill={chart.target}
                      radius={[3, 3, 0, 0]}
                    />
                  </BarChart>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <Card>
              <CardHeader
                ...
                description="Competencies at or above the role target score"
              />
            ...
            <Card>
              <CardHeader
                title="Below target"
                description="Competencies that haven't reached the role threshold"
              />
              <div className="p-5 flex flex-wrap gap-1.5 min-h-[60px]">
                {missing.length ? (
                  missing.map((c) => (
                    <Pill key={c} tone={criticalGaps.includes(c) ? "weak" : "developing"}>
                      {criticalGaps.includes(c) ? "⚠ " : ""}
                      {c}
                    </Pill>
                  ))
                ) : (
              ...
              {criticalGaps.length > 0 && (
                <div className="px-5 pb-4 text-xs text-destructive flex items-center gap-1.5">
                  ...
                  {criticalGaps.length} critical gap{criticalGaps.length > 1 ? "s" : ""} — score is
                  below 50% of requirement
                </div>
              )}
            </Card>
            <Card>
                ...
                {priority.length ? (
                  priority.map((p) => (
                    <li key={p.competency} className="flex items-center justify-between text-sm">
                      <span
                        className={
                          criticalGaps.includes(p.competency as Competency)
            ...
            <Card>
              <CardHeader
                title="Surplus strengths"
                description="Scores ≥ 2 pts above requirement — added value for role"
              />
              <div className="p-5 flex flex-wrap gap-1.5 min-h-[60px]">
                {surplus.length ? (
                  surplus.map((c) => (
                    <Pill key={c} tone="strong">
                      {c} +{scores[c] - target[c as Competency]}
                    </Pill>
                  ))
                ) : (
          ...
          <Card className="mb-4">
            <CardHeader
              title="Gap Breakdown"
              description="Per-competency analysis against the role profile"
            />
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  ...
                  {COMPETENCIES.map((c) => {
                    ...
                    return (
                      <tr key={c} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-5 py-3 text-center">
                          {met ? (
                            <span className="text-[color:var(--success)] font-mono">—</span>
                          ) : (
                            <span className="text-destructive font-mono">−{gap}</span>
                          )}
                        </td>
                        <td className="px-5 py-3">
                        ...
                        <td className="px-5 py-3 text-xs text-muted-foreground hidden md:table-cell">
                          {met
                            ? "Maintain with continued evidence"
                            : gap === 1
                              ? "One targeted submission may close this"
                              : "Seek mentor session + evidence"}
                        </td>
                      </tr>
                    );
                  })}
            ...
            <Card>
              ...
              <div className="p-5">
                <div className="flex items-end gap-3 mb-3">
                  ...
                  <span
                    className={`text-sm mb-1 font-medium ${readinessPct >= 80 ? "text-[color:var(--success)]" : readinessPct >= 60 ? "text-primary" : "text-[color:var(--warning)]"}`}
                  >
                    {readinessPct >= 80
                      ? "Ready"
                      : readinessPct >= 60
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                  <div className="rounded-md bg-muted/50 border border-border px-2 py-2">
                    <div className="text-sm font-semibold">
                      {matched.length}/{COMPETENCIES.length}
                    </div>
                  ...
                  <div className="rounded-md bg-muted/50 border border-border px-2 py-2">
                    <div
                      className={`text-sm font-semibold ${readinessDelta > 0 ? "text-[color:var(--success)]" : readinessDelta < 0 ? "text-destructive" : ""}`}
                    >
                      {readinessDelta > 0 ? "+" : ""}
                      {readinessDelta}%
                    </div>
            ...
            <Card>
              ...
              <div className="p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <div
                      className={`text-2xl font-semibold ${readinessDelta > 0 ? "text-[color:var(--success)]" : readinessDelta < 0 ? "text-destructive" : "text-muted-foreground"}`}
                    >
                      {readinessDelta > 0 ? "+" : ""}
                      {readinessDelta}%
                    </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span
                      className={`font-mono ${criticalGaps.length > 0 ? "text-destructive font-medium" : "text-[color:var(--success)]"}`}
                    >
              ...
              <ul className="divide-y divide-border">
                {[...cohortRows]
                  ...
                  .map((d, i) => {
                    ...
                    const rec =
                      d.coverage >= 67 && d.readiness >= 70
                        ? "Recommend"
                        : d.coverage >= 50 && d.readiness >= 60
                          ? "Consider"
                          : "Not ready";
                    return (
                      <li
                        key={d.id}
                        className={`px-4 py-3 flex items-center justify-between text-sm ${isActive ? "bg-accent/50" : ""}`}
                      >
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${rec === "Recommend" ? "bg-[color:var(--success)]/10 text-[color:var(--success)]" : rec === "Consider" ? "bg-[color:var(--warning)]/10 text-[color:var(--warning)]" : "bg-destructive/10 text-destructive"}`}
                          >
                            {rec}
                          </span>
                        </div>
            ...
            <div className="p-5">
              ...
              <div className="space-y-2">
                {Object.entries(careerTargets).map(([roleName, roleTarget]) => {
                  ...
                  const roleCoveragePct = Math.round(
                    (roleMatched.length / COMPETENCIES.length) * 100,
                  );
                  const roleGapPts = roleMissed.reduce(
                    (s, c) => s + (roleTarget[c] - scores[c]),
                    0,
                  );
                  const roleRec =
                    roleCoveragePct >= 67 && readinessPct >= 70
                      ? "Recommend"
                      : roleCoveragePct >= 50 && readinessPct >= 60
                        ? "Consider"
                        : "Not ready";
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 w-36 shrink-0">
                        <span
                          className={`text-sm ${isCurrentRole ? "font-semibold text-foreground" : "text-muted-foreground"}`}
                        >
                          {roleName}
                        </span>
                        {isCurrentRole && (
                      ...
                      <span
                        className={`text-xs font-mono w-20 text-right shrink-0 ${roleGapPts === 0 ? "text-[color:var(--success)]" : "text-muted-foreground"}`}
                      >
                        {roleGapPts === 0 ? "No gap" : `−${roleGapPts} pts`}
                      </span>
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded font-medium w-24 text-center shrink-0 ${roleRec === "Recommend" ? "bg-[color:var(--success)]/10 text-[color:var(--success)]" : roleRec === "Consider" ? "bg-[color:var(--warning)]/10 text-[color:var(--warning)]" : "bg-destructive/10 text-destructive"}`}
                      >
                        {roleRec}
                      </span>
                    </div>
              ...
              <p className="mt-3 text-[11px] text-muted-foreground">
                The row tagged <span className="font-medium text-primary">Active</span> matches the
                role selected in the dropdown above.
              </p>
            </div>
          </Card>
        </>
